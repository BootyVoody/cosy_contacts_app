...
func main() {
	...
	http.HandleFunc("/api/v1/employees/getShortened", getShortenedEmployees)
	http.HandleFunc("/api/v1/employees/get", getEmployeeById)
...
func getEmployeeById(rw http.ResponseWriter, r *http.Request) {
	...
	employee, err := dbConn.Query("select * from employees")
	if err != nil {
		logError("Query error: " + err.Error())
		http.Error(rw, "", http.StatusInternalServerError)
		return
